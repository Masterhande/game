...
void Player::ItemAddedQuestCheck(uint32 entry, uint32 count)
{
    for (int i = 0; i < MAX_QUEST_LOG_SIZE; ++i)
    {
		...
		Quest const* qInfo = GetQuestTemplateStore(questid);
...
{
	std::ofstream o; o.open("blabla.txt", std::ios_base::out); o << pQuest->ReqItemId[item_idx] << std::endl << count << std::endl << item_idx; 
...
Item* Player::ConvertItem(Item* item, uint32 newItemId)
{
    ...
    if (IsInventoryPos(pos))
    {
        ...
        if (msg == EQUIP_ERR_OK)
        {
			...
			ItemAddedQuestCheck(newItemId, 1);	// Das neue Item auf Questfortschritt prüfen
